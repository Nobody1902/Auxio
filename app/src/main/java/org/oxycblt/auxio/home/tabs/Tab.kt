...
sealed class Tab(open val type: MusicType) {
    ...
    companion object {
         ...
         * ordered as "Song", "Album", "Artist", "Genre", "Playlists", and "Lyrics"
         */
        ...
        fun fromIntCode(intCode: Int): Array<Tab>? {
            ...
            // Make sure there are no duplicate tabs (preferring the later occurrence,
            // which handles old format values that have a zero-padded leading entry)
            val distinct = tabs.reversed().distinctBy { it.type }.reversed()
            if (tabs.size != distinct.size) {
                L.w(
                    "Tab sequences should not have duplicates [old: ${tabs.size} new: ${distinct.size}]"
                )
